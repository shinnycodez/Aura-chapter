...
const Header = () => {
  ...
  return (
    <>
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#96A78D] bg-[#96A78D] px-4 md:px-10 py-3 relative">
        <div className="flex items-center gap-4 md:gap-8">
          <Link to="/" className="flex items-center gap-2 md:gap-4 text-[#141414]">
            <h2 className="text-[#141414] text-lg font-bold leading-tight tracking-[-0.015em]">JEMZI</h2>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-5 lg:gap-9">
            ...
            <Link to="/products?category=Bracelets" className="text-[#141414] text-sm font-medium transition">Bracelets ⋆౨ৎ˚⟡</Link>
            <Link to="/products?category=Cuffs" className="text-[#141414] text-sm font-medium  transition">
Cuffs ✮ ⋆ ˚｡
</Link>
            <Link to="/products?category=Earrings" className="text-[#141414] text-sm font-medium  transition">Earrings ⋆˚꩜｡</Link>
...
#96A78D] items-center justify-center pl-4 rounded-l-lg border-r-0">
...
#96A78D] focus:border-none h-full placeholder:text-neutral-500 px-4 rounded-l-none border-l-0 pl-2 text-base font-normal leading-normal"
                value={searchQuery}
...
#96A78D] text-[#141414] hover:bg-[#FFDCDC] transition">🛒 Cart</button>
            {user ? (
              <>
                <button onClick={handleLogout} className="px-4 h-10 rounded-lg bg-[#
#96A78D] text-[#141414] text-sm font-semibold hover:bg-[#
#96A78D] transition">Logout</button>
...
#96A78D] text-white text-sm font-semibold hover:opacity-90 transition"
              >
...
#96A78D]">
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg z-50 border-t border-[#96A78D]">
            <div className="px-4 py-2 space-y-2">
              ...
              <Link to="/products?category=Cuffs" onClick={() => setIsMenuOpen(false)} className="blocsk px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Cuffs ✮ ⋆ ˚｡
</Link>
