...
const Header = () => {
  ...
  return (
    <>
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#ea9594] bg-[#ea9594] px-4 md:px-10 py-3 relative">
        <div className="flex items-center gap-4 md:gap-8">
          <Link to="/" className="flex items-center gap-2 md:gap-4 text-[#141414]">
            <h2 className="text-[#fdf9ee] text-lg font-bold leading-tight tracking-[-0.015em]">Aura Chapter</h2>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-5 lg:gap-9">
            ...
            <Link to="/products?category=Mini canvases" className="text-[#141414] text-sm font-medium transition">Canvases ⋆౨ৎ˚⟡</Link>
            <Link to="/products?category=Resin Coasters" className="text-[#141414] text-sm font-medium  transition">
 Coasters ✮ ⋆ ˚｡
</Link>
            <Link to="/products?category=Resin pencil holder" className="text-[#141414] text-sm font-medium  transition">Pencil holder ⋆˚꩜｡</Link>
            <Link to="/products?category=Resin jar" className="text-[#141414] text-sm font-medium  transition">Jar ✧˖°.｡</Link>
...
#ea9594] items-center justify-center pl-4 rounded-l-lg border-r-0">
...
#ea9594] focus:border-none h-full placeholder:text-neutral-500 px-4 rounded-l-none border-l-0 pl-2 text-base font-normal leading-normal"
                value={searchQuery}
...
#ea9594] text-[#141414] hover:bg-[#FFDCDC] transition">🛒 Cart</button>
            {user ? (
              <>
                <button onClick={handleLogout} className="px-4 h-10 rounded-lg bg-[#
#ea9594] text-[#141414] text-sm font-semibold hover:bg-[#
#ea9594] transition">Logout</button>
...
#ea9594] text-white text-sm font-semibold hover:opacity-90 transition"
              >
...
#ea9594]">
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg z-50 border-t border-[#ea9594]">
            <div className="px-4 py-2 space-y-2">
              ...
              <Link to="/products?category=Mini canvases" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Mini canvases ˚⊹♡</Link>
              <Link to="/products?category=Resin coasters" onClick={() => setIsMenuOpen(false)} className="blocsk px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Resin coasters ✮ ⋆ ˚｡
</Link>
              <Link to="/products?category=Resin pencil holder" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">
Resin pencil holder˚⋆𐙚｡
</Link>
              <Link to="/products?category=Resin jar" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition">Resin jar ｡°✩
</Link>
